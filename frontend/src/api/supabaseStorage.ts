...
const PRODUCT_IMAGE_BUCKET = (
    process.env.EXPO_PUBLIC_SUPABASE_BUCKET_PRODUCTS ?? "product-images"
).trim();
...
const buildImagePath = (input: UploadProductImageInput) => {
    const baseName =
        safeSegment(input.sku) || safeSegment(input.productName) || "product";
    const ext =
        getExtFromName(input.fileName) ||
...
const getClient = () => {
    if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
        throw new Error(
            "Thiếu cấu hình Supabase. Vui lòng set EXPO_PUBLIC_SUPABASE_URL và EXPO_PUBLIC_SUPABASE_ANON_KEY.",
        );
    }
