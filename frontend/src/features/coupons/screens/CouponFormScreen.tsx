...
const CouponFormScreen = () => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                setStartsAt(res.data.startsAt ? res.data.startsAt.substring(0, 10) : "");
                setEndsAt(res.data.endsAt ? res.data.endsAt.substring(0, 10) : "");
    ...
    const handleSubmit = async () => {
        if (!code.trim()) { Alert.alert("Thiếu thông tin", "Vui lòng nhập mã coupon."); return; }
        if (!discountValue) { Alert.alert("Thiếu thông tin", "Vui lòng nhập giá trị giảm."); return; }
        const payload = {
        ...
        try {
            ...
            Alert.alert("Lỗi", err?.response?.data?.message || "Không thể lưu coupon.");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const valuePlaceholder = discountType === "PERCENT" ? "Ví dụ: 10 (= 10%)" : "Ví dụ: 50000 (= 50,000 VND)";
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.title}>{isEdit ? "Sửa mã giảm giá" : "Thêm mã giảm giá"}</Text>
            </View>

            <ScrollView style={styles.body} contentContainerStyle={{ paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
                <View style={styles.card}>
                    <Text style={styles.sectionTitle}>Thông tin mã giảm giá</Text>

                    <Text style={styles.label}>Mã coupon <Text style={styles.required}>*</Text></Text>
                    <TextInput style={styles.input} value={code} onChangeText={v => setCode(v.toUpperCase())}
                        placeholder="SUMMER10" placeholderTextColor={theme.colors.mutedForeground} autoCapitalize="characters" />

                    <Text style={styles.label}>Loại giảm giá <Text style={styles.required}>*</Text></Text>
                    <TouchableOpacity style={styles.picker} onPress={() => setShowTypeModal(true)}>
                    ...
                    <Text style={styles.label}>Giá trị giảm <Text style={styles.required}>*</Text></Text>
                    <TextInput style={styles.input} value={discountValue} onChangeText={setDiscountValue}
                        placeholder={valuePlaceholder} placeholderTextColor={theme.colors.mutedForeground} keyboardType="numeric" />
                </View>
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.rowInputs}>
                        <View style={{ flex: 1 }}>
                            ...
                            <TextInput style={styles.input} value={minOrderAmount} onChangeText={setMinOrderAmount}
                        ...
                        <View style={{ flex: 1 }}>
                            ...
                            <TextInput style={styles.input} value={maxDiscountAmount} onChangeText={setMaxDiscountAmount}
                                placeholder="Không giới hạn" placeholderTextColor={theme.colors.mutedForeground} keyboardType="numeric" />
                        </View>
                    </View>
                    ...
                    <View style={styles.rowInputs}>
                        <View style={{ flex: 1 }}>
                            ...
                            <TextInput style={styles.input} value={startsAt} onChangeText={setStartsAt}
                                placeholder="YYYY-MM-DD" placeholderTextColor={theme.colors.mutedForeground} />
                        </View>
                        <View style={{ flex: 1 }}>
                            <Text style={styles.label}>Ngày kết thúc</Text>
                            <TextInput style={styles.input} value={endsAt} onChangeText={setEndsAt}
                                placeholder="YYYY-MM-DD" placeholderTextColor={theme.colors.mutedForeground} />
                        </View>
                    </View>

                    <Text style={styles.label}>Giới hạn số lần dùng</Text>
                    <TextInput style={styles.input} value={usageLimit} onChangeText={setUsageLimit}
                        placeholder="Để trống = không giới hạn" placeholderTextColor={theme.colors.mutedForeground} keyboardType="numeric" />
            ...
            <View style={styles.footer}>
                ...
                >
                    ...
                    <Text style={styles.submitBtnText}>
                        {submitting ? "Đang lưu..." : isEdit ? "Cập nhật" : "Tạo mã giảm giá"}
                    </Text>
                </TouchableOpacity>
            </View>
                    ...
                    <View style={styles.modalBox}>
                        ...
                        {DISCOUNT_TYPES.map(t => (
                            <TouchableOpacity key={t.value}
                                style={[styles.modalItem, t.value === discountType && styles.modalItemSelected]}
                                onPress={() => { setDiscountType(t.value); setShowTypeModal(false); }}>
                                <Text style={[styles.modalItemName, t.value === discountType && { color: theme.colors.primary }]}>{t.label}</Text>
                                {t.value === discountType && <Feather name="check" size={16} color={theme.colors.primary} />}
                            </TouchableOpacity>
                        ))}
                    </View>
                </View>
