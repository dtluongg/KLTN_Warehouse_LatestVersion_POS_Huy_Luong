...
const GoodsReceiptFormScreen = () => {
            ...
            try {
                ...
                setPurchaseOrders(
                    poRows.filter(
                        (po: PurchaseOrder) =>
                            !po.closedAt &&
                            po.receiptProgress !== "FULLY_RECEIVED",
                    ),
                );
    ...
    const handleSelectPO = async (po: PurchaseOrder) => {
        ...
        try {
            ...
            if (poItems.length === 0) {
                Alert.alert(
                    "Không còn dòng nhận",
                    "PO này đã nhận đủ hoặc đã đóng.",
                );
                setItems([]);
                return;
            }
            ...
            setItems(
                poItems.map((item: POItem) => ({
                    ...
                    maxQty: Number(item.remainingQty ?? 0),
                })),
            );
        } catch (e) {
    ...
    useEffect(() => {
        ...
        const loadEdit = async () => {
            try {
                ...
                Alert.alert("Lỗi", "Không thể tải phiếu.");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async () => {
        if (!poId) {
            Alert.alert("Thiếu thông tin", "Chọn phiếu đặt hàng (PO).");
            return;
        }
        if (!warehouseId) {
            Alert.alert("Thiếu thông tin", "Chọn kho nhận hàng.");
            return;
        }
        if (items.length === 0) {
            Alert.alert("Thiếu sản phẩm", "Phiếu phải có ít nhất 1 sản phẩm.");
            return;
        }
        const invalidItem = items.find(
            (it) =>
                it.receivedQty <= 0 ||
                (it.maxQty != null && it.receivedQty > it.maxQty),
        );
        if (invalidItem) {
            Alert.alert(
        ...
        try {
            ...
            Alert.alert(
                "Lỗi",
                err?.response?.data?.message || "Không thể lưu phiếu.",
            );
        } finally {
            setSubmitting(false);
        }
            ...
            >
                <View style={styles.card}>
                    ...
                    <TouchableOpacity
                        ...
                        onPress={() =>
                            canEdit && !isEdit && setShowPoModal(true)
                        }
                ...
                <View style={styles.summaryCard}>
                    ...
                    <View style={styles.summaryRow}>
                        ...
                        <Text style={styles.summaryValue}>
                            +{" "}
                            {Number(surchargeAmount || 0).toLocaleString(
                                "vi-VN",
                            )}{" "}
                            đ
                        </Text>
                            ...
                            renderItem={({ item }) => (
                                ...
                                >
                                    ...
                                    <Text style={styles.modalItemSub}>
                                        {item.supplierName} · Kho:{" "}
                                        {item.warehouseName}
                                    </Text>
                                    <Text style={styles.modalItemSub}>
                                        Tiến độ:{" "}
                                        {item.receiptProgress || "NOT_RECEIVED"}
                                    </Text>
                                </TouchableOpacity>
                            )}
                        />
