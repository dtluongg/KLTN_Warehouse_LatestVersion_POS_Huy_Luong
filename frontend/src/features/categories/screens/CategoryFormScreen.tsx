...
const CategoryFormScreen = () => {
    ...
    const handleSubmit = async () => {
        if (!name.trim()) { Alert.alert("Thiếu thông tin", "Vui lòng nhập tên danh mục."); return; }
        if (!slug.trim()) { Alert.alert("Thiếu thông tin", "Vui lòng nhập slug."); return; }
        ...
        try {
            ...
            Alert.alert("Lỗi", err?.response?.data?.message || "Không thể lưu danh mục.");
        } finally {
            setSubmitting(false);
        }
    };
        ...
        <View style={styles.container}>
            ...
            <ScrollView style={styles.body} contentContainerStyle={{ paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
                <View style={styles.card}>
                    ...
                    <Text style={styles.label}>Slug (đường dẫn) <Text style={styles.required}>*</Text></Text>
                    <View style={styles.inputWithBtn}>
                        <TextInput
                            ...
                            autoCapitalize="none"
                        />
                        <TouchableOpacity style={styles.genBtn} onPress={() => setSlug(generateSlug(name))}>
