    ...
    const handlePickAndUploadImage = async () => {
        try {
            ...
            if (!permission.granted) {
                Alert.alert("Thiếu quyền", "Vui lòng cấp quyền truy cập thư viện ảnh để chọn ảnh sản phẩm.");
                return;
            }
            ...
            Alert.alert("Lỗi", err?.message || "Không thể upload ảnh sản phẩm.");
        } finally {
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.imageUploadBox}>
                        {localImageUri || imageUrl ? (
                            <Image source={{ uri: localImageUri || imageUrl }} style={styles.imagePreview} />
                        ) : (
                            <View style={styles.imagePlaceholder}>
                                <Feather name="image" size={24} color={theme.colors.mutedForeground} />
                                <Text style={styles.imagePlaceholderText}>Chưa có ảnh sản phẩm</Text>
                            </View>
                        )}

                        <View style={styles.imageActionsRow}>
                            <TouchableOpacity
                                style={[styles.imageActionBtn, uploadingImage && styles.imageActionBtnDisabled]}
                            ...
                            >
                                ...
                                <Text style={styles.imageActionText}>{uploadingImage ? "Đang upload..." : "Chọn & upload ảnh"}</Text>
                            </TouchableOpacity>

                            {(localImageUri || imageUrl) && (
                                <TouchableOpacity style={styles.imageRemoveBtn} onPress={handleClearImage} disabled={uploadingImage || submitting}>
                        ...
                        <Text style={styles.imageHint}>Ảnh sẽ được upload vào Supabase Storage. URL được lưu tự động khi bấm lưu sản phẩm.</Text>
                    </View>
