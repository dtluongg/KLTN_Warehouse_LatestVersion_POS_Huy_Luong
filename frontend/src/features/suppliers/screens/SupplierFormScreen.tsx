...
const SupplierFormScreen = () => {
    ...
    const handleAddProduct = async () => {
        if (!selectedProductId || !addingPrice || Number(addingPrice) < 0) {
            Alert.alert("Thiếu thông tin", "Vui lòng chọn sản phẩm và nhập giá.");
            return;
        }
        try {
    ...
    return (
        <View style={styles.container}>
                    ...
                    <View style={styles.card}>
                        ...
                        {loadingSP ? (
                            ...
                            <View style={styles.spEmpty}>
                                ...
                                <Text style={styles.spEmptyText}>Chưa có sản phẩm nào. Nhấn "Thêm SP" để bổ sung.</Text>
                            </View>
                        ) : (
                                    ...
                                    <View style={{ alignItems: "flex-end" }}>
                                        ...
                                        {sp.lastUpdatedAt && (
                                            <Text style={styles.spDate}>
                                                C/nh: {new Date(sp.lastUpdatedAt).toLocaleDateString("vi-VN")}
                                            </Text>
                                        )}
                                    </View>
                                    <TouchableOpacity
            ...
            <Modal visible={showAddProductModal} transparent animationType="slide" onRequestClose={() => setShowAddProductModal(false)}>
                <View style={styles.modalOverlay}>
                    <View style={styles.modalBox}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>Thêm sản phẩm vào bảng giá</Text>
                            <TouchableOpacity onPress={() => { setShowAddProductModal(false); setSelectedProductId(null); setAddingPrice(""); }}>
                            ...
                            <>
                                ...
                                <FlatList
                                    data={allProducts.filter((p: any) => {
                                        const inSearch = p.name.toLowerCase().includes(addProductSearch.toLowerCase()) ||
                                            p.sku.toLowerCase().includes(addProductSearch.toLowerCase());
                                        const alreadyAdded = supplierProducts.some(sp => sp.productId === p.id);
                                    ...
                                    renderItem={({ item }: any) => (
                                        <TouchableOpacity style={styles.modalItem} onPress={() => {
                                            ...
                                            setAddingPrice(String(item.lastPurchaseCost ?? item.avgCost ?? 0));
                                        }}>
                                    ...
                                    ListEmptyComponent={
                                        <Text style={{ textAlign: "center", padding: 20, color: theme.colors.mutedForeground }}>
                            ...
                            <View style={{ padding: 4 }}>
                                ...
                                <View style={{ flexDirection: "row", gap: 10, marginTop: 16 }}>
                                    <TouchableOpacity
                                        ...
                                        onPress={() => { setSelectedProductId(null); setAddingPrice(""); }}
                                    >
                                        <Text style={[styles.submitBtnText, { color: theme.colors.foreground }]}>Quay lại</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        style={[styles.submitBtn, { flex: 1 }, addingProduct && styles.submitBtnDisabled]}
                                    ...
                                    >
                                        ...
                                        <Text style={styles.submitBtnText}>{addingProduct ? "Đang lưu..." : "Xác nhận"}</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        )}
...
const styles = StyleSheet.create({
    ...
    spEmptyText: { color: theme.colors.mutedForeground, fontSize: 13, textAlign: "center" },
    spRow: {
        flexDirection: "row",
        alignItems: "center",
        gap: 8,
