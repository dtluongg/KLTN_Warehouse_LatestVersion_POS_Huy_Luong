...
const WarehouseFormScreen = () => {
    ...
    const handleSubmit = async () => {
        if (!code.trim()) { Alert.alert("Thiếu thông tin", "Vui lòng nhập mã kho."); return; }
        if (!name.trim()) { Alert.alert("Thiếu thông tin", "Vui lòng nhập tên kho."); return; }
        const payload = { code: code.trim(), name: name.trim(), address: address.trim() || null, isActive };
        try {
            setSubmitting(true);
            if (isEdit) {
            ...
            Alert.alert("Lỗi", err?.response?.data?.message || "Không thể lưu kho hàng.");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.title}>{isEdit ? "Sửa kho hàng" : "Thêm kho hàng"}</Text>
            </View>
            ...
            <ScrollView style={styles.body} contentContainerStyle={{ paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
                <View style={styles.card}>
                    ...
                    <Text style={styles.label}>Mã kho <Text style={styles.required}>*</Text></Text>
                    <TextInput
                        style={styles.input}
                        value={code}
                    ...
                    <Text style={styles.label}>Tên kho <Text style={styles.required}>*</Text></Text>
                    <TextInput
                        style={styles.input}
                        value={name}
            ...
            <View style={styles.footer}>
                ...
                >
                    ...
                    <Text style={styles.submitBtnText}>
                        {submitting ? "Đang lưu..." : isEdit ? "Cập nhật" : "Tạo kho hàng"}
                    </Text>
                </TouchableOpacity>
            </View>
        </View>
