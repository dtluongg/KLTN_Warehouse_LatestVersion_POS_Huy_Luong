...
const StaffFormScreen = () => {
    ...
    const handleSubmit = async () => {
        if (!fullName.trim()) { Alert.alert("Thiếu thông tin", "Vui lòng nhập họ tên nhân viên."); return; }
        if (!username.trim()) { Alert.alert("Thiếu thông tin", "Vui lòng nhập tên đăng nhập."); return; }
        if (!role) { Alert.alert("Thiếu thông tin", "Vui lòng chọn chức vụ."); return; }
        ...
        try {
            ...
            Alert.alert("Lỗi", err?.response?.data?.message || "Không thể lưu nhân viên.");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.title}>{isEdit ? "Sửa nhân viên" : "Thêm nhân viên"}</Text>
            </View>
            ...
            <ScrollView style={styles.body} contentContainerStyle={{ paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
                ...
                <View style={styles.card}>
                    ...
                    <Text style={styles.label}>Họ tên đầy đủ <Text style={styles.required}>*</Text></Text>
                    ...
                    <View style={styles.rowInputs}>
                        <View style={{ flex: 1 }}>
                            ...
                            <TextInput style={styles.input} value={phone} onChangeText={setPhone}
                                placeholder="0901234567" placeholderTextColor={theme.colors.mutedForeground} keyboardType="phone-pad" />
                        </View>
                        <View style={{ flex: 1 }}>
                            <Text style={styles.label}>Mã số thuế</Text>
                            <TextInput style={styles.input} value={taxCode} onChangeText={setTaxCode}
                                placeholder="MSCN..." placeholderTextColor={theme.colors.mutedForeground} />
                        </View>
                    </View>

                    <Text style={styles.label}>Email</Text>
                    <TextInput style={styles.input} value={email} onChangeText={setEmail}
                        placeholder="email@company.com" placeholderTextColor={theme.colors.mutedForeground}
                        keyboardType="email-address" autoCapitalize="none" />

                    <Text style={styles.label}>Địa chỉ</Text>
                    <TextInput style={[styles.input, styles.inputMultiline]} value={address} onChangeText={setAddress}
                        placeholder="Địa chỉ thường trú..." placeholderTextColor={theme.colors.mutedForeground}
                        multiline numberOfLines={2} />
                </View>

                {/* Tài khoản & Quyền hạn */}
                <View style={styles.card}>
                    <Text style={styles.sectionTitle}>Tài khoản & Quyền hạn</Text>

                    <Text style={styles.label}>Tên đăng nhập <Text style={styles.required}>*</Text></Text>
                    <TextInput style={[styles.input, isEdit && styles.inputLocked]} value={username} onChangeText={setUsername}
                        placeholder="username" placeholderTextColor={theme.colors.mutedForeground}
                        autoCapitalize="none" editable={!isEdit} />
                    ...
                    <View style={styles.passwordRow}>
                        <TextInput
                            ...
                            placeholder={isEdit ? "Nhập mật khẩu mới..." : "Mặc định: 123456"}
                            placeholderTextColor={theme.colors.mutedForeground}
                            secureTextEntry={!showPassword} autoCapitalize="none"
                        />
                        <TouchableOpacity style={styles.eyeBtn} onPress={() => setShowPassword(!showPassword)}>
                            <Feather name={showPassword ? "eye-off" : "eye"} size={18} color={theme.colors.mutedForeground} />
                        </TouchableOpacity>
                    </View>

                    <Text style={styles.label}>Chức vụ / Vai trò <Text style={styles.required}>*</Text></Text>
                    <TouchableOpacity style={styles.picker} onPress={() => setShowRoleModal(true)}>
                        <Feather name="shield" size={16} color={theme.colors.mutedForeground} />
                        <Text style={styles.pickerText}>{selectedRoleLabel}</Text>
                        <Feather name="chevron-down" size={16} color={theme.colors.mutedForeground} />
                    </TouchableOpacity>

                    <View style={styles.switchRow}>
                        <Text style={styles.label}>Tài khoản đang hoạt động</Text>
                        <TouchableOpacity
                            activeOpacity={0.9}
            ...
            <View style={styles.footer}>
                ...
                >
                    ...
                    <Text style={styles.submitBtnText}>
                        {submitting ? "Đang lưu..." : isEdit ? "Cập nhật" : "Tạo nhân viên"}
                    </Text>
                </TouchableOpacity>
            </View>
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalBox}>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) => (
                                <TouchableOpacity style={[styles.modalItem, item.value === role && styles.modalItemSelected]}
                                    onPress={() => { setRole(item.value); setShowRoleModal(false); }}>
                                    <Text style={[styles.modalItemName, item.value === role && { color: theme.colors.primary }]}>
                                        {item.label}
                                    </Text>
                                    {item.value === role && <Feather name="check" size={16} color={theme.colors.primary} />}
                                </TouchableOpacity>
                            )}
                        />
                    </View>
