...
const SupplierReturnDetailView = ({ id }: { id: number }) => {
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            try {
                const res = await axiosClient.get(`/supplier-returns/${id}`);
                setDetail(res.data);
            } catch (error) {
                console.error("Error fetching supplier return detail", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <View style={styles.detailContainer}>
                ...
                {detail.items?.map((item: any, index: number) => (
                    <View key={item.id || index} style={styles.itemRow}>
                        ...
                        <View style={styles.itemStats}>
                            ...
                            <Text style={styles.itemAmount}>{formatMoney(item.returnAmount)}</Text>
                        </View>
                    </View>
                ))}
            </View>
            ...
            <View style={[styles.summaryRow, { marginTop: 8, paddingTop: 8, borderTopWidth: 1, borderTopColor: theme.colors.border }]}>
                <Text style={[styles.summaryLabel, { fontWeight: '700', color: theme.colors.primary }]}>Cần thu từ NCC:</Text>
...
const SupplierReturnListScreen = () => {
    ...
    useEffect(() => {
        const fetchWarehouses = async () => {
            try {
                ...
                // Giả sử res.data là mảng object kho hàng
    ...
    return (
        <DataTableScreen
            ...
            createAction={{
                label: "Thêm trả NCC",
                onPress: () => navigation.navigate("SupplierReturnForm"),
            }}
            rowActions={[
                {
                    label: "Sửa",
                    tone: "neutral",
                    onPress: (row) => navigation.navigate("SupplierReturnForm", { id: row.id }),
            ...
            renderFilters={(setFilters, currentFilters) => {
                ...
                return (
                    <View style={{ gap: 20 }}>
                        {/* Time Filter */}
                        <View style={{ gap: 8 }}>
                            <Text style={{ fontSize: 13, fontWeight: '600', color: theme.colors.foreground }}>Thời gian tạo:</Text>
            ...
            columns={[
                ...
                { key: "goodsReceiptId", label: "ID Nhập", width: 100 },
