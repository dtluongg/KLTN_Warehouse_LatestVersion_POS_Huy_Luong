...
const GoodsReceiptListScreen = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                else if (whRes.data && Array.isArray(whRes.data.content))
                    setWarehouses(whRes.data.content);

                if (Array.isArray(supRes.data)) setSuppliers(supRes.data);
                else if (supRes.data && Array.isArray(supRes.data.content))
    ...
    const completeGoodsReceipt = async (row: any) => {
        const confirmed = await confirmAction(
            "Duyệt GR",
            `Duyệt ${row?.grNo || "phiếu"}?`,
            "Duyệt",
        );
        if (!confirmed) return;
    ...
    const cancelDraftGoodsReceipt = async (row: any) => {
        const confirmed = await confirmAction(
            "Hủy GR",
            `Hủy ${row?.grNo || "phiếu"}?`,
            "Hủy phiếu",
    ...
    return (
        <DataTableScreen
            ...
            rowActions={[
                {
                    ...
                    onPress: (row) =>
                        navigation.navigate("GoodsReceiptForm", { id: row.id }),
                },
                {
                    label: "In",
                    tone: "neutral",
                ...
                },
                {
                    label: "Hủy",
                    tone: "danger",
                    onPress: cancelDraftGoodsReceipt,
                    shouldShow: (row) =>
                        role === "ADMIN" &&
                        String(row?.status || "") === "DRAFT",
                },
            ...
            renderDetailContent={(row) => (
                <GoodsReceiptDetailView id={row.id} />
            )}
