...
const PurchaseOrderListScreen = () => {
    ...
    return (
        <DataTableScreen
            ...
            rowActions={[
                {
                    ...
                    onPress: (row) => navigation.navigate("PurchaseOrderForm", { id: row.id }),
                },
                {
                    label: "In",
                    tone: "neutral",
            ...
            renderDetailContent={(row) => <PurchaseOrderDetailView id={row.id} />}
            renderFilters={(setFilters, currentFilters) => {
                const statuses = ["DRAFT", "POSTED", "CANCELLED", "COMPLETED"];
                // Lưu ý: Purchase Order có thể có COMPLETED nếu nghiệp vụ cho phép
                ...
                return (
                    <View style={{ gap: 20 }}>
                        {/* Time Filter */}
                        <View style={{ gap: 8 }}>
                            <Text style={{ fontSize: 13, fontWeight: '600', color: theme.colors.foreground }}>Ngày đặt hàng:</Text>
