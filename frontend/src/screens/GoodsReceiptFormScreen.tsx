...
interface PurchaseOrder { id: number; poNo: string; supplierId: string; supplierName: string; warehouseId: number; warehouseName: string; }
interface POItem { id: number; productId: number; productName: string; productSku: string; orderedQty: number; expectedUnitCost: number; vatRate?: number; }
...
const GoodsReceiptFormScreen = () => {
    ...
    const vatAmount   = items.reduce((s, i) => s + i.receivedQty * i.unitCost * (i.vatRate / 100), 0);
        ...
        const load = async () => {
            try {
                const [poRes, wRes] = await Promise.all([
                    axiosClient.get("/purchase-orders?status=POSTED&size=200"),
                    axiosClient.get("/warehouses?size=100"),
                ]);
                setPurchaseOrders(poRes.data.content || poRes.data || []);
    ...
    const handleSelectPO = async (po: PurchaseOrder) => {
        ...
        } catch (e) { console.error("Load PO items:", e); }
    };
    ...
    useEffect(() => {
        ...
        const loadEdit = async () => {
            try {
                ...
                const d = res.data;
                setPoId(d.poId); setPoNo(d.poNo || "");
                setSupplierId(d.supplierId); setSupplierName(d.supplierName || "");
                setWarehouseId(d.warehouseId); setWarehouseName(d.warehouseName || "");
                setNote(d.note || "");
    ...
    const handleSubmit = async () => {
        if (!poId) { Alert.alert("Thiếu thông tin", "Chọn phiếu đặt hàng (PO)."); return; }
        if (!warehouseId) { Alert.alert("Thiếu thông tin", "Chọn kho nhận hàng."); return; }
        if (items.length === 0) { Alert.alert("Thiếu sản phẩm", "Phiếu phải có ít nhất 1 sản phẩm."); return; }
        const payload = {
        ...
        try {
            ...
            if (isEdit) {
                ...
                Alert.alert("Thành công", `Đã tạo phiếu ${res.data.grNo}.`);
            }
            navigation.goBack();
        } catch (err: any) {
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.title}>{isEdit ? "Sửa phiếu nhập hàng" : "Tạo phiếu nhập hàng (GR)"}</Text>
            </View>
            ...
            <ScrollView style={styles.body} contentContainerStyle={{ paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
                ...
                <View style={styles.card}>
                    ...
                    <Text style={styles.label}>Phiếu đặt hàng (PO) <Text style={styles.required}>*</Text></Text>
                    <TouchableOpacity
                        style={[styles.picker, (!canEdit || isEdit) && styles.pickerDisabled]}
                    ...
                    >
                        ...
                        <Text style={[styles.pickerText, !poId && styles.pickerPlaceholder]}>
                            {poId ? `${poNo} · ${supplierName}` : "Chọn phiếu đặt hàng..."}
                        </Text>
                        {(canEdit && !isEdit) && <Feather name="chevron-down" size={16} color={theme.colors.mutedForeground} />}
                    </TouchableOpacity>

                    {!!warehouseId && (
                        <>
                            <Text style={styles.label}>Kho nhận hàng</Text>
                            <TouchableOpacity
                            ...
                            >
                                ...
                                <Text style={styles.pickerText}>{warehouseName}</Text>
                                    ...
                                    <View style={styles.lineItemInputRow}>
                                        <View style={{ flex: 1 }}>
                                            <Text style={styles.lineInputLabel}>
                                                Số lượng{item.maxQty ? ` (tối đa: ${item.maxQty})` : ""}
                                            </Text>
                                            <TextInput
                                                style={[styles.lineInput, !canEdit && styles.inputDisabled]}
                                                keyboardType="numeric" value={String(item.receivedQty)}
                                                onChangeText={v => updateItem(idx, "receivedQty", Number(v) || 0)} editable={canEdit}
                                            />
                                        </View>
                                        <View style={{ flex: 1.5 }}>
                                            ...
                                            <TextInput
                                                style={[styles.lineInput, !canEdit && styles.inputDisabled]}
                                                keyboardType="numeric" value={String(item.unitCost)}
                                                onChangeText={v => updateItem(idx, "unitCost", Number(v) || 0)} editable={canEdit}
                                            />
                                        </View>
                    ...
                    <View style={styles.rowInputs}>
                        <View style={{ flex: 1 }}>
                            ...
                            <TextInput style={[styles.input, !canEdit && styles.inputDisabled]}
                                keyboardType="numeric" value={discountAmount} onChangeText={setDiscountAmount} editable={canEdit} />
                        </View>
                        <View style={{ flex: 1 }}>
                            <Text style={styles.label}>Phụ phí (VND)</Text>
                            <TextInput style={[styles.input, !canEdit && styles.inputDisabled]}
                                keyboardType="numeric" value={surchargeAmount} onChangeText={setSurchargeAmount} editable={canEdit} />
                        </View>
                    </View>
                </View>
            ...
            {canEdit && (
                <View style={styles.footer}>
                    <TouchableOpacity style={[styles.submitBtn, submitting && styles.submitBtnDisabled]}
                        ...
                        <Text style={styles.submitBtnText}>{submitting ? "Đang lưu..." : isEdit ? "Cập nhật phiếu" : "Tạo phiếu nhập hàng"}</Text>
                    </TouchableOpacity>
                </View>
            )}

            {/* PO Picker Modal */}
            <Modal visible={showPoModal} transparent animationType="slide" onRequestClose={() => setShowPoModal(false)}>
                <View style={styles.modalOverlay}><View style={styles.modalBox}>
                    ...
                    <TextInput style={styles.modalSearch} placeholder="Tìm mã PO, nhà cung cấp..."
                        value={poSearch} onChangeText={setPoSearch} placeholderTextColor={theme.colors.mutedForeground} />
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <TouchableOpacity style={styles.modalItem} onPress={() => handleSelectPO(item)}>
                                <Text style={styles.modalItemName}>{item.poNo}</Text>
                                <Text style={styles.modalItemSub}>{item.supplierName} · Kho: {item.warehouseName}</Text>
                            </TouchableOpacity>
                        )}
                    />
                </View></View>
...
const styles = StyleSheet.create({
    ...
    lineItemHeader: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 6 },
    lineItemName: { fontSize: 14, fontWeight: "600", color: theme.colors.foreground, flex: 1 },
    lineItemSku: { fontSize: 12, color: theme.colors.mutedForeground, marginBottom: 10 },
