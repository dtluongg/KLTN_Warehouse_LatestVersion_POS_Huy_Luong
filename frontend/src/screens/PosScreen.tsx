...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, TextInput, ActivityIndicator, Alert, ScrollView, useWindowDimensions, Image } from 'react-native';
...
const MOCK_CUSTOMERS = [
  { id: 'KH001', name: 'Nguyễn Thị Nông' },
  { id: 'KH002', name: 'Trần Văn Dân' },
];
...
export const PosScreen = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Trả dữ liệu từ Backend xuống State
  ...
  const filteredCustomers = MOCK_CUSTOMERS.filter(c => {
    ...
    const phone = c.id === 'KH001' ? '0901234567' : '0912345678'; 
    return c.name.toLowerCase().includes(kw) || phone.includes(kw);
  });
  ...
  const handleCheckout = async () => {
    ...
    // Bỏ qua Fake IDs (KH001) => Đổi thành null nếu không phải UUID hợp lệ
    const validCustomerId = (customerId && uuidRegex.test(customerId)) ? customerId : null;
    ...
    try {
      ...
      Alert.alert('Thành công', `Đã tạo Đơn hàng #${res.data.orderNo}\nKhách phải trả: ${res.data.netAmount.toLocaleString('vi-VN')} đ`);
      clearCart();
      if (!isLargeScreen) setMobileTab('PRODUCTS');
      
      // Reload lại danh sách sản phẩm để cập nhật Tồn kho
      fetchProducts();
    } catch (error) {
      console.log('Lỗi thanh toán:', error);
      Alert.alert('Lỗi', 'Không gởi được Đơn hàng lên Server. Vui lòng thử lại!');
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderProductsArea = () => (
    <View style={[styles.productsArea, !isLargeScreen && { flex: 1 }]}>
      {/* Header & Search */}
      <View style={[styles.headerArea, !isLargeScreen && { flexDirection: 'column', alignItems: 'flex-start', gap: 12 }]}>
        <Text style={styles.title}>Danh Sách Sản Phẩm</Text>
        <View style={[styles.searchBar, !isLargeScreen && { width: '100%' }]}>
          ...
          {searchKeyword.length > 0 && (
            <TouchableOpacity onPress={() => setSearchKeyword('')}>
              <Feather name="x-circle" size={18} color={theme.colors.mutedForeground} />
            </TouchableOpacity>
          )}
        </View>
      </View>
      ...
      <View style={{ marginBottom: theme.spacing.lg }}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
          {MOCK_CATEGORIES.map(cat => (
            ...
            >
              <Text style={[styles.categoryChipText, activeCategoryId === cat.id && { color: theme.colors.primaryForeground }]}>
                {cat.name}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={{ textAlign: 'center', marginTop: 40, color: theme.colors.mutedForeground }}>Không tìm thấy sản phẩm nào.</Text>}
          renderItem={({ item }) => (
            <TouchableOpacity 
              style={[styles.productCard, item.onHand <= 0 && { opacity: 0.5 }, !isLargeScreen && { maxWidth: '48%' }]} 
              activeOpacity={0.7}
              onPress={() => handleAddToCart(item)}
            >
              {item.imageUrl ? (
                <Image source={{ uri: item.imageUrl }} style={styles.productImagePlaceholder} />
              ) : (
                <View style={styles.productImagePlaceholder}>
                  <Feather name="box" size={32} color={theme.colors.mutedForeground} />
                </View>
              )}
              <View style={styles.productInfo}>
                ...
                <View style={styles.productFooter}>
                  <Text style={styles.productPrice}>{item.salePrice.toLocaleString('vi-VN')} đ</Text>
                  <Text style={[styles.productStock, item.onHand <= 0 && { color: theme.colors.error, fontWeight: '700' }]}>
                    SL: {item.onHand}
                  </Text>
                </View>
              </View>
  ...
  const renderCartArea = () => (
    <View style={[styles.cartArea, !isLargeScreen && { flex: 1, borderLeftWidth: 0 }]}>
      <View style={styles.cartHeader}>
        <Text style={styles.cartTitle}>Giỏ Hàng {cart.length > 0 && `(${cart.length})`}</Text>
        <TouchableOpacity onPress={clearCart}>
          <Text style={{ color: theme.colors.error, fontSize: 14, fontWeight: '600' }}>Xóa hết</Text>
        </TouchableOpacity>
      </View>

      {/* Banner kho đang chọn */}
      <TouchableOpacity style={styles.warehouseBanner} onPress={() => setShowWarehouseModal(true)}>
        ...
        <Feather name="chevron-down" size={14} color={theme.colors.mutedForeground} />
      </TouchableOpacity>
      ...
      >
          ...
          {customerId ? (
            <View style={styles.selectedCustomerBox}>
              <Text style={styles.selectedCustomerName} numberOfLines={1}>
                {customerId === 'KH001' ? 'Nguyễn Thị Nông' : customerId === 'KH002' ? 'Trần Văn Dân' : 'Khách lẻ'}
              </Text>
                    ...
                    filteredCustomers.map(c => (
                      ...
                      >
                        ...
                        <Text style={{ fontSize: 12, color: theme.colors.mutedForeground }}>
                          {c.id === 'KH001' ? '0901234567' : '0912345678'}
                        </Text>
                      </TouchableOpacity>
                    ))
                  ) : (
                    <Text style={{ padding: 12, color: theme.colors.mutedForeground }}>Không tìm thấy khách hàng.</Text>
                  )}
                </View>
              )}
            </View>
        ...
        {cart.length === 0 ? (
          <View style={{ alignItems: 'center', paddingVertical: 40 }}>
            <Feather name="shopping-cart" size={48} color={theme.colors.muted} />
            <Text style={{ marginTop: 16, color: theme.colors.mutedForeground }}>Giỏ hàng rỗng</Text>
          </View>
        ) : (
            ...
            {cart.map((item) => (
              <View key={item.id} style={styles.cartItem}>
                <View style={styles.cartItemInfo}>
                  ...
                  <Text style={styles.cartItemPrice}>{item.salePrice.toLocaleString('vi-VN')} đ</Text>
                </View>
        ...
        {cart.length > 0 && (
          <View style={styles.invoiceModifiers}>
            ...
            <View style={styles.modifierRow}>
              <Text style={styles.modifierLabel}>Chiết khấu (VND):</Text>
              <TextInput 
            ...
            {/* Coupon Code */}
            <View style={styles.modifierRow}>
              <Text style={styles.modifierLabel}>Mã giảm (+%):</Text>
              <TextInput 
                style={styles.numericInput} 
                placeholder="Nhập mã..."
                value={couponCode}
                onChangeText={(val) => {
                  setCoupon(val, val === 'VIP10' ? (getGrossAmount() * 0.1) : 0); // Demo code VIP10 giảm 10%
                }}
              />
            </View>
            {/* Phụ phí */}
            <View style={styles.modifierRow}>
              <Text style={styles.modifierLabel}>Phụ phí / Ship (VND):</Text>
              <TextInput 
                style={styles.numericInput} 
                keyboardType="numeric" 
                placeholder="0"
            ...
            {/* Ghi chú */}
            <View style={styles.modifierRow}>
              <Text style={styles.modifierLabel}>Ghi chú đơn:</Text>
              <TextInput 
                style={[styles.numericInput, { textAlign: 'left' }]} 
          ...
          <View style={styles.inputSection}>
            ...
            <View style={styles.paymentMethodRow}>
              {['CASH', 'TRANSFER', 'MIX', 'DEBT'].map(method => (
                ...
                >
                  <Text style={paymentMethod === method ? { color: '#fff', fontSize: 12, fontWeight: '700' } : { fontSize: 12, color: theme.colors.foreground, fontWeight: '500' }}>
      ...
      <View style={styles.cartFooter}>
        <View style={styles.summaryRow}>
          ...
          <Text style={styles.summaryValue}>{getGrossAmount().toLocaleString('vi-VN')} đ</Text>
        </View>
        {(discountAmount > 0 || couponCode.length > 0) && (
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Tiền giảm giá</Text>
            <Text style={[styles.summaryValue, { color: theme.colors.error }]}>
              - {(discountAmount + (couponCode === 'VIP10' ? getGrossAmount() * 0.1 : 0)).toLocaleString('vi-VN')} đ
            </Text>
          </View>
        )}
        {surchargeAmount > 0 && (
          <View style={styles.summaryRow}>
            ...
            <Text style={styles.summaryValue}>+ {surchargeAmount.toLocaleString('vi-VN')} đ</Text>
          </View>
        )}
        <View style={[styles.summaryRow, styles.summaryTotalRow]}>
          <Text style={styles.totalLabel}>Khách Cần Trả</Text>
          <Text style={styles.totalValue}>{getNetAmount().toLocaleString('vi-VN')} đ</Text>
        </View>
  ...
  const renderWarehouseModal = () => (
    <View style={[styles.modalOverlay, !showWarehouseModal && { display: 'none' }]}>
      <View style={styles.modalBox}>
        ...
        <Text style={styles.modalSubtitle}>Vui lòng chọn kho trước khi bắt đầu bán hàng</Text>
        <View style={styles.warehouseList}>
          {warehouses.length === 0 ? (
            ...
            warehouses.filter(w => w.isActive).map(w => (
              <TouchableOpacity
                key={w.id}
                style={[styles.warehouseItem, warehouseId === w.id && styles.warehouseItemActive]}
                onPress={() => {
                  setWarehouseId(w.id);
              ...
              >
                <View style={{ flex: 1 }}>
                  <Text style={[styles.warehouseItemName, warehouseId === w.id && { color: theme.colors.primary }]}>
                    {w.name}
                  </Text>
                  {w.address ? (
                    <Text style={styles.warehouseItemAddress} numberOfLines={1}>{w.address}</Text>
                  ) : null}
                </View>
                {warehouseId === w.id && (
        ...
        {warehouseId && (
          <TouchableOpacity style={styles.modalConfirmBtn} onPress={() => setShowWarehouseModal(false)}>
    ...
    <View style={[styles.container, { flexDirection: isLargeScreen ? 'row' : 'column' }]}>
      ...
      {!isLargeScreen && (
        <View style={styles.mobileTabWrapper}>
          <TouchableOpacity 
            style={[styles.mobileTabBtn, mobileTab === 'PRODUCTS' && styles.mobileTabBtnActive]}
            onPress={() => setMobileTab('PRODUCTS')}
          >
            <Text style={[styles.mobileTabText, mobileTab === 'PRODUCTS' && { color: theme.colors.primary }]}>
              HÀNG HÓA
            </Text>
          </TouchableOpacity>
          <TouchableOpacity 
          ...
          >
            <Text style={[styles.mobileTabText, mobileTab === 'CART' && { color: theme.colors.primary }]}>
      ...
      {isLargeScreen ? (
        ...
        // Mobile View
        mobileTab === 'PRODUCTS' ? renderProductsArea() : renderCartArea()
      )}
...
const styles = StyleSheet.create({
  ...
  qtyBtn: { padding: 6 },
  qtyText: { fontSize: 13, fontWeight: '600', minWidth: 20, textAlign: 'center' },
  ...
  summaryValue: { color: theme.colors.foreground, fontWeight: '600', fontSize: 14 },
  summaryTotalRow: {
    marginTop: 8,
    paddingTop: 12,
    borderTopWidth: 1,
  ...
  totalLabel: { color: theme.colors.foreground, fontWeight: '700', fontSize: 16 },
  totalValue: { color: theme.colors.primary, fontWeight: '800', fontSize: 22 },
  checkoutBtn: {
    backgroundColor: theme.colors.primary,
    padding: theme.spacing.md,
    borderRadius: theme.borderRadius.md,
