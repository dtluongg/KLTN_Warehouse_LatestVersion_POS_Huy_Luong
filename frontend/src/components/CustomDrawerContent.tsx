...
type MenuItem = { label: string; icon: string; route: string; isHighlight?: boolean };
...
const MENU_SECTIONS: MenuSection[] = [
  {
    section: null,
    items: [
      { label: 'Tổng quan', icon: 'grid', route: 'Overview' },
    ],
  },
  {
    section: 'BÁN HÀNG',
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={styles.roleBadge}>
              <Text style={styles.roleText}>
                {role === 'ADMIN' ? 'Quản trị viên' : role === 'SALES_STAFF' ? 'Bán hàng' : 'Kho'}
              </Text>
            </View>
          </View>
        </View>
          ...
          {MENU_SECTIONS.map((group, groupIndex) => (
            <View key={groupIndex}>
              ...
              {group.items.map((item, itemIndex) => {
                ...
                const itemColor = isActive ? theme.colors.primary : theme.colors.mutedForeground;
                return (
                  <TouchableOpacity
                    key={itemIndex}
                    style={[
                      ...
                      item.isHighlight && !isActive && styles.menuItemHighlight,
                    ]}
                  ...
                  >
                    <Feather
                      ...
                      size={18}
                      color={item.isHighlight && !isActive ? theme.colors.primary : itemColor}
                    />
                    <Text style={[
                      styles.menuLabel,
                      isActive && styles.menuLabelActive,
                      item.isHighlight && !isActive && { color: theme.colors.primary, fontWeight: '600' },
                    ]}>
                      {item.label}
                    </Text>
                  </TouchableOpacity>
